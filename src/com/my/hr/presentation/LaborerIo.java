...
public class LaborerIo {
	...
	private void fixLaborer() {
		if(laborerService.getLaborers().size() > 0) {
			...
			if(laborerId > 0) {
				String laborerName = Console.inStr("수정 할 노동자명을 입력하세요.", 5);
				LocalDate hireDate = Console.inDate("수정 할 입사일을 입력하세요.");
				
				laborerService.fixLaborer(new Laborer(laborerId, laborerName, hireDate));
				Console.info("수정을 완료했습니다.");
			}
		} else Console.info("수정 할 노동자가 없습니다.");
	}
	...
	private void delLaborer() {
		...
		} else Console.info("삭제 할 노동자가 없습니다.");
	}
	...
	private int getLaborer(String job) {
		...
		do {
			...
			if(laborer == null) Console.info("해당하는 ID가 없습니다.");
		} while(laborer == null);
